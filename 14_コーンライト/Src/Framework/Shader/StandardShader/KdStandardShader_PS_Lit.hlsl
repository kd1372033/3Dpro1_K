...
float4 main(VSOutput In) : SV_Target0
{
	...
	if (g_ConeLight.enable)
	{
		...
		if (len < g_ConeLight.range)
		{
			...
			// 内積
			// toDir=今から塗ろうとしているピクセルからコーンライトの発射位置への方向ベクトル
			// g_ConeLight.dir=コーンライトの発射方向
			// saturate = 飽和（０～１にしてくれる）
			float rad = acos(saturate(dot(toDir, -g_ConeLight.dir))); //  -g_ConeLight.dirは逆ベクトルにする
			if (rad < g_ConeLight.angle)
			{
				...
				// 減衰率（attenuation → atte）
				float atte = saturate(1.0 - len / g_ConeLight.range) * side;

				// 光の方向と法線の方向との角度さが光の強さになる
				// wN = ピクセルの法線
				float lightDiffuse = dot(wN, toDir);
				lightDiffuse = saturate(lightDiffuse); // マイナス値は0にする　0(暗)～1(明)になる
